...
// Проверяет, имеет ли авторизованный пользователь оплату в статусах (Не оплачен или Оплачен) к указанному курсу.
...
export default defineEventHandler(async (event) => {
    ...
    try {
        const response = await $fetch(`${apiUrl}?method=checkCourseBuy&act=auth`, {
            method: 'POST',
            headers: {
        ...
        if ('PAY_EXISTS' in response && typeof response.PAY_EXISTS === 'number') {
            return {
                PAY_EXISTS: response.PAY_EXISTS
            }
        }
        ...
        console.error('Ошибка при проверке оплаты курса:', error)
        const errorMessage = error.message || 'Неизвестная ошибка'
        let userFriendlyMessage = 'Ошибка при проверке оплаты курса'
        let statusCode = 400
