...
export default defineEventHandler(async (event) => {
    ...
    // console.log('Add to favorites request:', { itemId })
    ...
    try {
        const response = await $fetch(`${apiUrl}?method=addfavorite&act=auth`, {
            method: 'POST',
            headers: {
        ...
        // console.log('Bitrix API response (add favorite):', JSON.stringify(response, null, 2))

        if (response.MESSAGE === 'FAVORITE_ADDED_SUCCESSFULLY') {
            return {
        ...
        let userFriendlyMessage = 'Ошибка при добавлении в избранное'
        ...
        switch (errorMessage) {
            ...
            case 'ERROR_INVALID_ITEM_ID':
                ...
                break
            case 'ERROR_ITEM_ALREADY_FAVORITE':
                userFriendlyMessage = 'Элемент уже в избранном'
                break
