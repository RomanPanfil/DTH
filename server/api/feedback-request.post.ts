...
export default defineEventHandler(async (event) => {
    ...
    // Проверка на наличие обязательного поля PHONE
    if (!body.phone) {
        return {
            error: 'Не удалось отправить заявку',
            details: 'Поле Телефон обязательно для заполнения'
        }
    }
    ...
    try {
        const response = await $fetch(`${apiUrl}?method=FeedbackRequest&act=add`, {
            method: 'POST',
            headers: {
