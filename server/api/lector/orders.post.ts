// Получение заказов связанных с лектором
...
export default defineEventHandler(async (event) => {
    ...
    try {
        const response = await $fetch(`${apiUrl}?method=getLectorOrders&act=auth`, {
            method: 'POST',
            headers: {
        ...
        console.error('Ошибка при получении заказов лектора:', error)
        const errorMessage = error.message || 'Неизвестная ошибка'
        let userFriendlyMessage = 'Ошибка при получении заказов лектора'
        let statusCode = 400
