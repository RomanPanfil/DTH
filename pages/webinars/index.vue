                    ...
                    <div class="filter">
                        ...
                        <div class="filter-section">
                            <label class="filter-label">
                                {{ getFilterLabel('PRICE_FOR_ALL') }}
                            </label>
                            <div class="filter-range">
                                <label
                                    v-for="range in getFilterRange('PRICE_FOR_ALL')"
...
const tempIsFree = ref<boolean>(false);
...
const hasActiveFilters = computed(() => {
    ...
    const priceFilter = filters.value.find((f) => f.CODE === 'PRICE_FOR_ALL');
    if (priceFilter) {
        if (
...
// Получение диапазона для фильтра (например, PRICE_FOR_ALL)
const getFilterRange = (code: string) => {
    const filter = filters.value.find((f) => f.CODE === 'PRICE_FOR_ALL');
...
const tempSelectDirection = (controlName: string, value?: string) => {
    ...
    const directionFilter = filters.value.find((f) => f.CODE === 'DIRECTION');
    if (directionFilter && formatFilter.VALUES) {
        Object.values(directionFilter.VALUES).forEach((val: any) => {
...
// Склонение слова "вебинар"
const getCourseWord = (count: number | undefined) => {
    if (count === undefined || count === null) return 'вебинаров';
    const absCount = Math.abs(count);
    const lastTwoDigits = absCount % 100;
    const lastDigit = absCount % 10;
    if (lastTwoDigits >= 11 && lastTwoDigits <= 14) return 'вебинаров';
    if (lastDigit === 1) return 'вебинар';
    if (lastDigit >= 2 && lastDigit <= 4) return 'вебинара';
    return 'вебинаров';
};
...
const loadFilters = async () => {
    try {
        ...
        eventsError.value = { details: error.message || 'Не удалось загрузки фильтры' };
    }
};
...
const applyFilters = () => {
    ...
    Object.keys(filterValues.value).forEach((key) => {
        if (filterValues.value[key] && filterValues.value[key] !== false && key !== 'fastfilter') {
...
const { data: eventsData, refresh: refreshEvents } = await useAsyncData(
    ...
    async () => {
        try {
            ...
            const filter = { IBLOCK_ID: iblockId, ACTIVE: 'Y', PROPERTY_MAIN_LESSON: 1 };
            const query = { page: currentPage.value, iblockId, GET_ALL_FILES: 'Y', locale: locale.value, PROPERTY_MAIN_LESSON: 1 };
            if (isFree.value) {
                filter.PROPERTY_IS_FREE = 1;
